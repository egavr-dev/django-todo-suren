...
Соответственно то что в теге, оно, не изменяется, а то что в блоках
мы можем менять.

Имя блока может быть любым, хоть например main, главное понимать что это блок
и потом переопределяя его, писать правильно его имя и любой блок закрывается.
Потому что внутри блоков, тоже могут быть другие блоки.

### Теперь добавим навигационную панель.
Часто сверху на сайтах есть navbar. Поэтому, снова меняем наш базовый шаблон
файл base.html
...
И вот у нас может быть огромный navbar, с полем поиска, и прочим, прочим.
...
Теперь у нас navbar вынесен в отдельный файл (компонент)
Преимущество такого подхода в том, что при изменении именно компонента,
нам не нужно менять базовый шаблон, а мы изменим сам компонент.

Документация по блоку include тут [link](https://docs.djangoproject.com/en/5.1/ref/templates/builtins/#include)
